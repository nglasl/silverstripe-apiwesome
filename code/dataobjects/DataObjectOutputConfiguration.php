...
class DataObjectOutputConfiguration extends DataObject
{
    ...
    public static function customise_data_objects($type, $objects = null)
    {
        ...
        } elseif (is_array($objects) && (strtolower($type) === 'include')) {
            self::$custom_inclusions = array_unique(array_merge(self::$custom_inclusions, $objects));
        } elseif (strtolower($type) === 'disabled') {
            self::$disabled = true;
    ...
    public function requireDefaultRecords()
    {
        ...
        foreach ($objects as $object) {
            ...
            elseif (!$existing && !self::$disabled && (get_parent_class($object) === 'DataObject') && !ClassInfo::classImplements($object, 'TestOnly') && (((count($inclusions) > 0) && in_array($object, $inclusions)) || ((count($inclusions) === 0) && !in_array($object, $exclusions)))) {
